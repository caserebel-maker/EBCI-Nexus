...
function statusClass(status: ExpensePaymentStatus) {
  ...
  if (status === "pending")
    return "border-yellow-300/35 bg-yellow-400/15 text-yellow-100";
  if (status === "not_eligible")
    return "border-slate-300/30 bg-white/10 text-white/70";
...
export function EmployeeExpensesCard({ employeeId, benefits, canEdit }: Props) {
  ...
  async function handleAddBenefit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(
      `/api/hradmin/employees/${employeeId}/expenses`,
      {
        method: "POST",
  ...
  async function handleAddPayment(
    ...
    setError(null);
    const response = await fetch(
      `/api/hradmin/employees/${employeeId}/expenses/${benefitId}/payments`,
      {
        method: "POST",
  ...
  async function updateBenefit(
    ...
    setError(null);
    const response = await fetch(
      `/api/hradmin/employees/${employeeId}/expenses/${benefitId}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    setEditingBenefitId((current) => (current === benefitId ? null : current));
    refresh();
  }
  ...
  async function deletePayment(benefitId: string, paymentId: string) {
    ...
    setError(null);
    const response = await fetch(
      `/api/hradmin/employees/${employeeId}/expenses/${benefitId}/payments/${paymentId}`,
      {
        method: "DELETE",
      },
    );
    ...
    <section className="rounded-[28px] border border-white/20 bg-white/10 p-5 text-white shadow-xl backdrop-blur">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/65">
              บันทึกค่าใช้จ่ายที่บริษัทตกลงจ่ายให้ เช่น ค่าโทรศัพท์ ค่าเดินทาง
              หรือสวัสดิการเฉพาะบุคคล
            </p>
          </div>
        </div>
        {canEdit && (
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-yellow-300/20 bg-yellow-400/10 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-yellow-100/70">
            Pending
          </p>
          <p className="mt-2 text-3xl font-bold text-yellow-100">
            {stats.pending}
          </p>
      ...
      {canEdit && isAddingBenefit && (
        <form
          ...
          className="mt-5 rounded-2xl border border-cyan-300/25 bg-black/20 p-4"
        >
          <div className="grid gap-3 md:grid-cols-2">
            <label className="space-y-1 text-sm">
              ...
              <input
                name="title"
                required
                placeholder="เช่น ค่าโทรศัพท์รายเดือน"
                className="w-full rounded-xl border border-white/15 bg-black/30 px-3 py-2 text-white outline-none focus:border-cyan-300"
              />
            </label>
            <label className="space-y-1 text-sm">
          ...
          <label className="mt-3 block space-y-1 text-sm">
            ...
            <textarea
              name="description"
              rows={2}
              placeholder="เงื่อนไขหรือรายละเอียดเพิ่มเติม"
              className="w-full rounded-xl border border-white/15 bg-black/30 px-3 py-2 text-white outline-none focus:border-cyan-300"
            />
          </label>
        ...
        {benefits.length === 0 ? (
          ...
          benefits.map((benefit) => {
            ...
            return (
              <article
                key={benefit.id}
                className="rounded-2xl border border-white/15 bg-black/15 p-4"
              >
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-semibold ${benefit.is_active ? "bg-emerald-400/20 text-emerald-100" : "bg-white/10 text-white/50"}`}
                      >
                ...
                {isEditingBenefit && (
                  <form
                    onSubmit={(event) => handleUpdateBenefit(event, benefit.id)}
                    className="mt-4 rounded-2xl border border-cyan-300/20 bg-cyan-400/10 p-3"
                  >
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="space-y-1 text-xs">
                        ...
                        <input
                          ...
                          required
                          className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-white outline-none focus:border-cyan-300"
                        />
                      </label>
                      <label className="space-y-1 text-xs">
                        <span className="font-semibold text-white/70">
                      ...
                      <label className="space-y-1 text-xs">
                        ...
                        <input
                          name="default_amount"
                          inputMode="decimal"
                          defaultValue={benefit.default_amount ?? ""}
                          className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-white outline-none focus:border-cyan-300"
                        />
                      </label>
                      <div className="grid grid-cols-2 gap-3">
                        <label className="space-y-1 text-xs">
                          ...
                          <input
                            name="start_month"
                            type="month"
                            defaultValue={benefit.start_month ?? ""}
                            className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-white outline-none focus:border-cyan-300"
                          />
                        </label>
                        <label className="space-y-1 text-xs">
                          ...
                          <input
                            name="end_month"
                            type="month"
                            defaultValue={benefit.end_month ?? ""}
                            className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-white outline-none focus:border-cyan-300"
                          />
                        </label>
                      </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                      <label className="inline-flex items-center gap-2 text-sm font-semibold text-white/75">
                        <input
                          name="is_active"
                          type="checkbox"
                          defaultChecked={benefit.is_active}
                          className="h-4 w-4 rounded border-white/30 bg-black/30 text-cyan-400"
                        />
                        ใช้งานรายการนี้
                      </label>
                      <button
                  ...
                  >
                    ...
                    <label className="space-y-1 text-xs">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white file:mr-2 file:rounded-lg file:border-0 file:bg-white/15 file:px-2 file:py-1 file:text-white"
                      />
                    </label>
                    <label className="space-y-1 text-xs md:col-span-5">
                      ...
                      <input
                        name="notes"
                        placeholder="รายละเอียด เช่น รอบบิล/เลขที่ใบเสร็จ"
                        className="w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-white outline-none"
                      />
                    </label>
                    <button
                      type="submit"
                      className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-400 px-3 py-2 text-sm font-bold text-slate-950 hover:bg-emerald-300"
                    >
                ...
                <div className="mt-4 overflow-hidden rounded-2xl border border-white/10">
                  ...
                  {benefit.payments.length === 0 ? (
                    ...
                    benefit.payments.map((payment) => (
                      <div
                        key={payment.id}
                        className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr_auto] items-center gap-2 border-t border-white/10 px-3 py-2 text-sm"
                      >
                        <span>
                          {formatExpenseMonth(
                        ...
                        <span
                          className={`w-fit rounded-full border px-2 py-1 text-xs font-semibold ${statusClass(payment.status)}`}
                        >
                        ...
                        <span className="truncate text-white/60">
                          {payment.notes ||
                            (payment.paid_on
                              ? `จ่ายวันที่ ${payment.paid_on}`
                              : "—")}
                        </span>
                        <span className="flex items-center justify-end gap-2">
                          {payment.receipt_path ? (
                            <a
                              href={`/api/hradmin/employees/${employeeId}/expenses/${benefit.id}/payments/${payment.id}`}
                              className="rounded-lg bg-white/10 p-2 text-cyan-100 hover:bg-white/15"
                          ...
                          {isEditingBenefit && (
                            <button
                              type="button"
                              onClick={() =>
                                deletePayment(benefit.id, payment.id)
                              }
                              className="rounded-lg bg-rose-500/15 p-2 text-rose-100 hover:bg-rose-500/25"
                              title="ลบรายการ"
                            >
