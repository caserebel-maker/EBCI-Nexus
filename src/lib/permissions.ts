...
// Server-side: resolve permissions of the currently logged-in user from the session cookie.
...
export async function getCurrentPermissions(): Promise<UserPermissions> {
    ...
    const { data } = await supabaseAdmin
        .from('User')
        .select('can_view_all_employees, can_edit_employees, can_view_approval_limits, can_edit_approval_limits, can_approve_leave, can_manage_system')
        .eq('id', session.id)
        .maybeSingle()
