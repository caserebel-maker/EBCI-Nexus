export const EMPLOYEE_LEVELS: Record<number, { label: string; color: string }> = {
    ...
    4: { label: 'Level 4 — HR Admin',                     color: 'bg-amber-500/20 text-amber-300 border-amber-500/30' },
    5: { label: 'Level 5 — MD (กรรมการผู้จัดการ)',       color: 'bg-red-500/20 text-red-300 border-red-500/30' },
    6: { label: 'Level 6 — ประธาน / เจ้าของ',             color: 'bg-yellow-500/20 text-yellow-200 border-yellow-400/40' },
}
...
export const LEVEL_BADGE_COLORS: Record<number, string> = {
    ...
    6: 'bg-yellow-500/20 text-yellow-200 border-yellow-400/40',
}
