...
function mapEmployeeCode(code: string): string {
    ...
    // Map Khanittha (Cream) Sarasureeporn's physical card codes to her DB employee_code '436-62'
    if (['748867', '48867', '048867', '488'].includes(compact) || compact === '48867' || trimmed === '488-67') {
        return '436-62'
    }
    ...
    if (compact.length === 6 && compact.startsWith('7')) {
        ...
        const mapped = `${slice.slice(0, 3)}-${slice.slice(3)}`
        if (mapped === '488-67') return '436-62'
        return mapped
    }
    // Pattern: 51369 -> 513-69 (format XXX-YY directly)
    if (compact.length === 5 && /^\d{5}$/.test(compact)) {
        const mapped = `${compact.slice(0, 3)}-${compact.slice(3)}`
        if (mapped === '488-67') return '436-62'
        return mapped
    }
    if (trimmed === '488-67') return '436-62'
    return trimmed
}
