...
// Detailed Mock Data (Phase 3.9)
const EMPLOYEE_DETAILS: Record<string, any> = {
    "1": {
        id: "1",
        employeeId: "EMP-001",
        ...
        type: "Full-time",
        supervisor: "Suriya Pond",
        emergencyContact: "Wipa Jai-dee (Wife) - +66 89 876 5432",
        lastLogin: "2026-01-30 09:15 AM",
        leaveBalance: { sick: 5, annual: 12, personal: 2 },
        attendance: "98.5%",
        notes: "Excellent performance in recruitment phase integration."
    },
...
const getEmployeeData = (id: string) => {
    // Ensure id exists and is a string for padStart
    const safeId = String(id || "0")

    return EMPLOYEE_DETAILS[safeId] || {
        id: safeId,
        employeeId: `EMP-${safeId.padStart(3, '0')}`,
        name: "Generic Employee",
        email: "employee@ebci.co.th",
        ...
        lastLogin: "2026-01-30 08:00 AM",
        leaveBalance: { sick: 30, annual: 6, personal: 3 },
        attendance: "100%",
        notes: "System generated profile."
    }
}

export default async function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params
    const data = getEmployeeData(id)
        ...
        <div className="space-y-6 max-w-6xl mx-auto">
                ...
                <div className="lg:col-span-2 flex flex-col md:flex-row gap-6 bg-card p-6 rounded-xl border border-white/20 dark:border-border shadow-sm">
                    ...
                    <div className="flex-1 space-y-4">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                            <div>
                                <h1 className="text-3xl font-bold text-foreground font-sans tracking-tight">{data.name}</h1>
                ...
                <div className="bg-white/15 dark:bg-primary/10 rounded-xl border border-white/30 dark:border-primary/20 p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <p className="text-2xl font-black text-white dark:text-foreground">{data.attendance}</p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-[10px] uppercase font-bold text-white/90 dark:text-muted-foreground/70">Leave (Annual)</p>
                            <p className="text-2xl font-black text-white dark:text-foreground">{data.leaveBalance.annual}d</p>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-card rounded-xl border border-white/10 dark:border-border p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        ...
                        <InfoRow label="Employee Type" value={data.type} icon={User} />
                        <InfoRow label="Direct Supervisor" value={data.supervisor} icon={Shield} />
                ...
                <div className="bg-card rounded-xl border border-white/10 dark:border-border p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-4 mt-4 border-t border-border">
                            ...
                            <p className="text-sm text-card-foreground font-medium">{data.emergencyContact}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-card rounded-xl border border-white/10 dark:border-border p-6 shadow-sm">
                ...
                <div className="bg-muted/30 border border-dashed border-border p-4 rounded-lg text-sm text-muted-foreground italic">
                    {data.notes || "No internal notes recorded for this employee."}
                </div>
